    ...
    <section className="py-16 sm:py-20 bg-background">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div ref={headingRef} className="flex items-end justify-between mb-10">
          <div>
            <span className="text-xs font-semibold uppercase tracking-widest text-[#c9a227] mb-2 block">Moments</span>
            <h2 className="text-2xl sm:text-3xl font-bold">Gallery</h2>
          </div>
          <a href="/gallery" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors inline-flex items-center gap-1.5">
            View All
        ...
        <div ref={ref} className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {items.length === 0 ? (
            ...
            <div
              key={item.id}
              className={`group rounded-2xl overflow-hidden bg-muted cursor-pointer hover:shadow-lg transition-all duration-300 ${
                i === 0 ? "md:col-span-2 md:row-span-2" : ""
              }`}
            >
              <div className={`bg-muted overflow-hidden ${i === 0 ? "aspect-[4/3]" : "aspect-video"}`}>
                {item.coverImage ? (
                  <img
                    src={item.coverImage}
                    alt={item.title}
              ...
              <div className={`${i === 0 ? "p-5" : "p-3"}`}>
                <div className={`font-semibold group-hover:text-[#c9a227] transition-colors ${i === 0 ? "text-lg" : "text-sm"}`}>
                  {item.title}
                </div>
