...
function AudienceField() {
  ...
  return (
    <div className="space-y-1.5">
      <label className="text-sm font-medium leading-none">Audience<span className="text-destructive ml-0.5">*</span></label>
      <Select value={audience} onValueChange={(val) => { if (val !== undefined) form.setFieldValue("audience", val) }} items={[{ value: "all", label: "Everyone" }, { value: "students", label: "Students" }, { value: "parents", label: "Parents" }, { value: "staff", label: "Staff" }, { value: "alumni", label: "Alumni" }]}>
...
function AddressedToField() {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <Input value={addressedTo} onChange={(e) => form.setFieldValue("addressedTo", e.target.value)} placeholder="e.g. Grade 11 Students, Science Faculty..." />
      <p className="text-xs text-muted-foreground">Optional: specify who this announcement is specifically for</p>
    </div>
  )
}
...
export function AnnouncementForm({ mode, id, onSuccess }: { mode: "create" | "edit"; id?: string; onSuccess: () => void }) {
  ...
  const createMutation = useMutation({
    mutationFn: (body: CreateAnnouncementValues) => client.announcements.create({ ...body, audience: body.audience as "all" | "students" | "parents" | "staff" | "alumni" }),
    onSuccess: () => { toast.success("Announcement created"); queryClient.invalidateQueries({ queryKey: ["announcements"] }); onSuccess() },
    onError: (err) => toast.error(err.message),
  })

  const updateMutation = useMutation({
    mutationFn: (body: UpdateAnnouncementValues & { id: string }) => client.announcements.update({ ...body, audience: body.audience as "all" | "students" | "parents" | "staff" | "alumni" }),
    onSuccess: () => { toast.success("Announcement updated"); queryClient.invalidateQueries({ queryKey: ["announcements"] }); onSuccess() },
    onError: (err) => toast.error(err.message),
  })

  const handleSubmit = useCallback(async (values: CreateAnnouncementValues | UpdateAnnouncementValues) => {
    if (mode === "edit" && id) await updateMutation.mutateAsync({ ...values, id } as UpdateAnnouncementValues & { id: string })
  ...
  if (mode === "edit" && !announcement) return <div className="p-4 text-center text-muted-foreground">Announcement not found.</div>
