...
export function OBCommitteeEditor({
  ...
  const saveMutation = useMutation(
    orpc.ob.obMembers.saveCommittee.mutationOptions({
      onSuccess: (res) => {
        toast.success(
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold tracking-[0.08em] text-foreground">
      ...
      {COMMITTEE_STRUCTURE.map((section) => {
        const sectionHasAny = section.roles.some(
          (r) => slots.filter((s) => s.role === r.role).length > 0,
        );
        if (readOnly && !sectionHasAny) return null;
        return (
          <Card key={section.section} className="overflow-hidden">
            <div className="border-b bg-muted/30 px-5 py-3">
              <h3 className="text-sm font-bold tracking-[0.1em] text-foreground">
            ...
            <CardContent className="space-y-4">
              {section.roles.map((r) => {
                ...
                if (readOnly && roleSlots.length === 0) return null;
                return (
                  <div key={r.role} className="space-y-2.5">
                    {roleSlots.length === 0 ? (
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <IconPlus className="size-3.5 mr-1" /> Assign {r.label}
                        </Button>
                      </div>
                    ) : (
                        ...
                        readOnly ? (
                          <div key={slot.key} className="flex items-center gap-3">
                            ...
                            <div className="flex min-w-0 flex-1 items-center gap-2.5">
                              <div className="relative size-9 shrink-0 overflow-hidden rounded-full bg-muted ring-1 ring-border">
                                {slot.photo ? (
                              ...
                              <span className="truncate text-sm font-medium text-green-dark">
                                {slot.name || "—"}
                              </span>
                            </div>
                          </div>
                    ...
                    {!readOnly && roleSlots.length > 0 && (
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
                        className="ml-[51px] text-xs text-muted-foreground"
                        onClick={() => addSlot(r.role)}
                      >
