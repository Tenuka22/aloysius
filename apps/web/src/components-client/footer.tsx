...
import { IconMapPin, IconPhone, IconMail, IconArrowUp, IconSend } from "@tabler/icons-react";
...
function FooterLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link
      to={to}
      className="group relative inline-flex w-fit items-center text-sm text-cream/65 hover:text-cream transition-colors"
    >
      {children}
      ...
      <div className="relative mx-auto max-w-295 px-4 sm:px-6 lg:px-12 pt-24 sm:pt-32 pb-10">
        ...
        >
          <div>
            ...
            {motto && (
              <p className="font-heading italic text-xl sm:text-2xl text-gold mt-2">{motto}</p>
            )}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-5 border-t border-cream/10 pt-7 text-[12.5px] text-cream/45">
          <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-5 text-center sm:text-left">
            <span>
              {copyright ||
                `\u00A9 ${new Date().getFullYear()} ${schoolName}, Galle. All Rights Reserved.`}
            </span>
