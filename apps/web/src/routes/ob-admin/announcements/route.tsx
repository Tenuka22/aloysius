...
export const Route = createFileRoute("/ob-admin/announcements")({
  validateSearch: (search) => announcementsSearchSchema.parse(search),
  loaderDeps: ({ search: { status, search } }) => ({ status, search }),
  loader: async ({ context, deps }) => {
    await context.queryClient.prefetchQuery(
      orpc.ob.obAnnouncements.list.queryOptions({ input: deps }),
    );
  },
  component: OBAdminAnnouncements,
});

function OBAdminAnnouncements() {
  ...
  const { data: announcements = [] } = useSuspenseQuery(
    orpc.ob.obAnnouncements.list.queryOptions({ input: search }),
  );

  const deleteMutation = useMutation(
    orpc.ob.obAnnouncements.delete.mutationOptions({
      onSuccess: () => {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                render={<Link to={`/ob-admin/announcements/$id/edit`} params={{ id: a.id }} />}
              >
    ...
    <div className="flex flex-col">
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4">
        ...
        <div className="ml-auto">
          <Button size="sm" render={<Link to="/ob-admin/announcements/new" />} nativeButton={false}>
