...
function ActionsMenu({ item }: { item: PageItem }) {
  const queryClient = useQueryClient()
  const [deleteOpen, setDeleteOpen] = useState(false)

  const deleteMutation = useMutation({
    mutationFn: () => client.pages.delete({ id: item.id }),
    onSuccess: () => {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem render={<Link to="/admin/pages/$id/edit" params={{ id: item.id }} />}>
...
const columns: ColumnDef<PageItem, any>[] = [
  {
    accessorKey: "title",
...
function AdminPagesList() {
  ...
  const status = typeof rawStatus === "string" && rawStatus.length > 0 ? (rawStatus as "draft" | "published") : undefined

  const { data, isLoading } = useQuery({
    queryKey: ["pages", pagination.pageIndex, pagination.pageSize, sort?.id, sort?.desc, search, status],
