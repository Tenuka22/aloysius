...
export const Route = createFileRoute("/admin/ob/news")({
  component: AdminOBNews,
});

function AdminOBNews() {
  ...
  const { data: articles = [], isLoading } = useQuery(orpc.ob.obNews.list.queryOptions({ input: {} }));

  const deleteMutation = useMutation(
    orpc.admin.ob.news.delete.mutationOptions({
      onSuccess: () => {
  ...
  const publishMutation = useMutation(
    orpc.admin.ob.news.update.mutationOptions({
      onSuccess: () => {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                render={<Link to={`/admin/ob/news/$id/edit`} params={{ id: a.id }} />}
              >
  ...
  return (
    <div className="flex flex-col">
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4">
        ...
        <h1 className="text-lg font-semibold">OB News</h1>
        <div className="ml-auto">
          <Button size="sm" render={<Link to="/admin/ob/news/new" />} nativeButton={false}>
