...
import { IconPlus, IconDotsVertical, IconPencil, IconTrash, IconPhoto, IconSend, IconArchive, IconRotate } from "@tabler/icons-react"
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem render={<Link to="/admin/gallery/$id/edit" params={{ id: item.id }} />}>
            <IconPencil className="size-4" />
            Edit
          </DropdownMenuItem>
          <DropdownMenuItem render={<Link to="/admin/gallery/$id/images" params={{ id: item.id }} />}>
...
const columns: ColumnDef<GalleryItem, any>[] = [
  ...
  },
  {
    ...
    header: "Event",
    cell: ({ row }) => (
      <span className="text-muted-foreground line-clamp-1">{row.original.eventId ?? "—"}</span>
    ),
  },
  {
    accessorKey: "tags",
...
function AdminGalleryList() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["gallery", pagination.pageIndex, pagination.pageSize, sort?.id, sort?.desc, search, status],
    queryFn: () => client.gallery.list({
    ...
    <div className="flex flex-col">
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4">
        ...
        <div className="ml-auto">
          <Button size="sm" render={<Link to="/admin/gallery/new" />} nativeButton={false}>
