...
type ActivityRow = { id: string; name: string; description: string | null; coverImage: string | null; type: string }
type BigMatchRow = { id: string; name: string; opponent: string; type: string; year: number | null; eventId: string | null; galleryId: string | null }
...
function AboutPage() {
  const { settings: settingsRaw, stats: statsRaw, activities, bigMatches: bigMatchesRaw } = Route.useLoaderData()
      ...
      <main id="main-content">
        ...
        <section ref={heroRef} className="px-4 sm:px-6 lg:px-8 pt-16 pb-12">
          <div className="mx-auto max-w-5xl text-center">
            <div data-animate className="inline-flex items-center gap-2 rounded-full bg-muted px-4 py-2 mb-6">
              <span className="text-sm font-medium">{getSetting("about_hero_badge")}</span>
            </div>
            <h1 data-animate className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight mb-4">
              {getSetting("about_hero_title")}
            </h1>
            <p data-animate className="text-xl text-muted-foreground mb-2">{getSetting("about_hero_location")}</p>
        ...
        <section ref={missionRef} className="px-4 sm:px-6 lg:px-8 py-16 border-t">
          <div className="mx-auto max-w-5xl">
            <h2 data-animate className="text-2xl sm:text-3xl font-bold text-center mb-12">{getSetting("about_mission_title")}</h2>
            <div className="grid md:grid-cols-2 gap-8">
              <div data-animate className="p-6 rounded-xl border bg-card">
                <h3 className="font-semibold text-lg mb-3">{getSetting("about_mission_jesuit_title")}</h3>
                <p className="text-muted-foreground">
                  {getSetting("about_mission_jesuit_desc")}
                </p>
              </div>
              <div data-animate className="p-6 rounded-xl border bg-card">
                <h3 className="font-semibold text-lg mb-3">{getSetting("about_mission_saint_title")}</h3>
                <p className="text-muted-foreground">
                  {getSetting("about_mission_saint_desc")}
                </p>
              </div>
            </div>
          </div>
        ...
        <section ref={valuesRef} className="px-4 sm:px-6 lg:px-8 py-16 border-t bg-muted/30">
          <div className="mx-auto max-w-5xl">
            <h2 className="text-2xl sm:text-3xl font-bold text-center mb-12">{getSetting("about_values_title")}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {values.map((value) => (
                <div key={value.title} data-animate className="p-6 rounded-xl border bg-card text-center">
        ...
        <section ref={clubsRef} className="px-4 sm:px-6 lg:px-8 py-16 border-t">
          <div className="mx-auto max-w-6xl">
            <h2 className="text-2xl sm:text-3xl font-bold text-center mb-4">{getSetting("about_clubs_title")}</h2>
            <p className="text-center text-muted-foreground mb-12 max-w-2xl mx-auto">
              {getSetting("about_clubs_desc")}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {clubs.map((club) => (
                <div key={club.id} data-animate className="group rounded-xl border bg-card overflow-hidden hover:shadow-md transition-shadow">
                  {club.coverImage ? (
                    ...
                    <div className="aspect-video bg-muted flex items-center justify-center">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="size-8 text-muted-foreground/40">
        ...
        <section ref={sportsRef} className="px-4 sm:px-6 lg:px-8 py-16 border-t bg-muted/30">
          <div className="mx-auto max-w-6xl">
            <h2 className="text-2xl sm:text-3xl font-bold text-center mb-4">{getSetting("about_sports_title")}</h2>
            <p className="text-center text-muted-foreground mb-12 max-w-2xl mx-auto">
              {getSetting("about_sports_desc")}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sports.map((sport) => (
                <div key={sport.id} data-animate className="group rounded-xl border bg-card overflow-hidden hover:shadow-md transition-shadow">
                  {sport.coverImage ? (
                    <div className="aspect-video overflow-hidden">
                      <img
        ...
        <section className="px-4 sm:px-6 lg:px-8 py-16 border-t">
          <div className="mx-auto max-w-5xl">
            <h2 className="text-2xl sm:text-3xl font-bold text-center mb-12">{getSetting("about_bigmatches_title")}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {bigMatches.map((match) => (
                <div key={match.id} className="p-6 rounded-xl border bg-card text-center">
                  <div className="text-xs text-muted-foreground uppercase tracking-wide mb-2">{match.type}</div>
                  <h3 className="font-semibold text-lg mb-1">{match.name}</h3>
                  <p className="text-sm text-muted-foreground">vs {match.opponent}</p>
                  {match.year && (
                    <p className="text-xs text-muted-foreground mt-2">{match.year}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        <section className="px-4 sm:px-6 lg:px-8 py-16 border-t bg-muted/30">
          <div className="mx-auto max-w-5xl">
            <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8">{getSetting("about_location_title")}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="p-6 rounded-xl border bg-card">
                <h3 className="font-semibold mb-3">Address</h3>
                <p className="text-muted-foreground">
                  {locationAddress.map((line, i) => (
                    <span key={i}>{line}{i < locationAddress.length - 1 && <br />}</span>
                  ))}
                </p>
              </div>
              <div className="p-6 rounded-xl border bg-card">
                ...
                <p className="text-muted-foreground">
                  Phone: <a href={`tel:${getSetting("about_location_phone").replace(/\s/g, "")}`} className="hover:text-foreground transition-colors">{getSetting("about_location_phone")}</a><br />
                  Email: <a href={`mailto:${getSetting("about_location_email")}`} className="hover:text-foreground transition-colors">{getSetting("about_location_email")}</a><br />
                  <a href={`https://${getSetting("about_location_website")}`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">
        ...
        <section className="px-4 sm:px-6 lg:px-8 py-16 border-t">
          <div className="mx-auto max-w-5xl text-center">
            <h2 className="text-2xl sm:text-3xl font-bold mb-4">{getSetting("about_alumni_title")}</h2>
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
              {getSetting("about_alumni_desc")}
            </p>
            <div className="flex justify-center gap-4 flex-wrap">
              {alumniCountries.map((country) => (
                <span key={country} className="inline-flex items-center rounded-full bg-muted px-4 py-2 text-sm font-medium">
                  {country}
                </span>
              ))}
            </div>
