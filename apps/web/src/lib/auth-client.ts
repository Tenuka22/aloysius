...
// Mirrors the server's `admin()` + `multiSession()` plugins (see
// packages/auth/src/index.ts) so `authClient.useSession()` is typed with
// `role`, and the admin/multi-session actions and error codes exist on this
// client. The two sides must be kept in sync plugin-for-plugin - there's no
